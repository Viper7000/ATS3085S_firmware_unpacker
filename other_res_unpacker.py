print("===================================================")
print("Распаковщик ресурсов UI для Смарт часов на ATS3085S")
print("===================================================")
...
with open(file_path, "rb") as f, open(map_output, "w", encoding="utf-8") as map_f:
    ...
    if not version.startswith(b'V'):
        print("Ошибка: Заголовок архива не начинается с буквы 'V'!")
        exit()
    
    version_str = version.decode('ascii', errors='ignore').strip()
    print(f"Обнаружена версия архива: {version_str}")
    
    # Записываем версию архива в начало файла и делаем отступ строки
    map_f.write(f"# Обнаружена версия архива: {version_str}\n\n")
    ...
    # Формируем текст статистики строго по шаблону с префиксом #
